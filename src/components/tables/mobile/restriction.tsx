...
export default function MobilePriceRestriction({
  data,
  handleDelete,
}: {
  data: restriction[];
  ...
  return (
    <div className=" w-full flex flex-col gap-4">
      <div className=" flex items-center justify-between py-3 font-semibold text-gray-500 text-sm bg-gray-100  px-3">
        <span>Min No. of Nights</span>
        <span>Max No. of Nights</span>
      </div>
      {data.map((item, index) => {
        return (
              ...
              <div className="text-sm flex flex-col">
                ...
                >
                  <div className={`flex items-center gap-3`}>
                    ...
                    />
                    <p className="">{item?.min_no_of_nights}</p>
                  </div>
                  <p>{item?.max_no_of_nights}</p>
                </Disclosure.Button>
