...
import sampleRReferralData from "../../assets/temp-api-mockup-data/generator-runtime.json";

//axios instace interceptor for access token integration and refresh tokens
export default function useGetALlGeneratorRuntimeReports({
  page = 1,
  start_date,
  end_date,
  sort = "asc",
  ...
  } = useAppSelector((state) => state.generatorRuntimeReporteports.value);
  ...
  const getAllCustomerstList = useCallback(async () => {
    ...
    try {
      ...
      if (foundPage && !remakeRequest) {
        ...
        dispatch(updateGeneratorRuntimeReport({ data: foundPage?.data }));
      } else {
        ...
        dispatch(updateGeneratorRuntimeReport({ data }));
        if (!remakeRequest) {
          dispatch(
            addToPaginationHistory({
