...
import tempAptData from "../assets/temp-api-mockup-data/priceTypes.json";
import {
  updatePriceTypeList,
  addToPaginationHistory,
} from "../stores/apiData/price-type-lists";
...
export default function useGetPriceTypeLists({ page = 1 }: { page?: number }) {
  ...
  } = useAppSelector((state) => state.allPriceTypes.value);
  ...
  const getAllPriceTypes = useCallback(async () => {
    setIsLoading(true);
    try {
  ...
  return {
    ...
    isFailed,
    setIsFailed,
    retryFunction: getAllPriceTypes,
    pagination,
  };
}
