...
export const requestListsData = createSlice({
  ...
  reducers: {
    updateRequestsLists: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addRequestsToList: (state, action) => {
    ...
    removeRequestsInList: (state, action) => {
    ...
    replaceRequestsInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: string }) => v.id === id
...
export const {
  ...
  removeRequestsInList,
  replaceRequestsInList,
  clearRequestsLists,
} = requestListsData.actions;

export default requestListsData.reducer;
