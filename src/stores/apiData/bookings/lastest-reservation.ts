...
export const latestReservation = createSlice({
  name: "latest-reservations",
  initialState: {
    value: {
      status: false,
      pagination: [] as {
  ...
  reducers: {
    updateLatestReservationList: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addLatestReservationToList: (state, action) => {
    ...
    removeLatestReservationInList: (state, action) => {
    ...
    replaceLatestReservationInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => String(v.id) === String(id)
    ...
    clearLatestReservationList: (state) => {
...
export const {
  ...
  addLatestReservationToList,
  addToPaginationHistory,
  removeLatestReservationInList,
  replaceLatestReservationInList,
  clearLatestReservationList,
} = latestReservation.actions;

export default latestReservation.reducer;
