  ...
  initialState: {
    value: {
      ...
      pagination: [] as {
        ...
        data: generatorRuntimeReport[];
      }[],
      data: [] as generatorRuntimeReport[],
    },
  },
  reducers: {
    updateGeneratorRuntimeReport: (state, action) => {
    ...
    removeGeneratorRuntimeInList: (state, action) => {
    ...
    replaceGeneratorRuntimeInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => String(v.id) === String(id)
    ...
    clearGeneratorRuntimeReport: (state) => {
