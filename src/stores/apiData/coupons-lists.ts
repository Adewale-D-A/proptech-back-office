...
export const couponListsData = createSlice({
  name: "all coupons",
  initialState: {
    value: {
      status: false,
  ...
  reducers: {
    updateCouponsList: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addCouponsToList: (state, action) => {
    ...
    removeCouponsInList: (state, action) => {
    ...
    replaceCouponsInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => String(v.id) === String(id)
...
export const {
  ...
  clearCouponsList,
} = couponListsData.actions;

export default couponListsData.reducer;
