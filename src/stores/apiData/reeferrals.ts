...
export const referralsListData = createSlice({
  ...
  reducers: {
    updateReferralsList: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addReferralsToList: (state, action) => {
    ...
    removeReferralsInList: (state, action) => {
    ...
    replaceReferralsInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => String(v.id) === String(id)
...
export const {
  ...
  removeReferralsInList,
  replaceReferralsInList,
  clearReferralsList,
} = referralsListData.actions;

export default referralsListData.reducer;
