...
export const requestsCategoriesData = createSlice({
  name: "requests_categroies_data",
  initialState: {
    value: {
      status: false,
      pagination: [] as {
  ...
  reducers: {
    updateRequestCategory: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addRequestCategory: (state, action) => {
    ...
    removeRequestCategory: (state, action) => {
    ...
    replaceRequestCategory: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => v.id === id
...
export const {
  ...
  removeRequestCategory,
  replaceRequestCategory,
  clearRequestCategory,
} = requestsCategoriesData.actions;

export default requestsCategoriesData.reducer;
