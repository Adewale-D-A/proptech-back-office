...
export const adminsListData = createSlice({
  name: "all-admins",
  initialState: {
    value: {
      status: false,
  ...
  reducers: {
    updateAdminsList: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addAdminsToList: (state, action) => {
    ...
    removeAdminsInList: (state, action) => {
    ...
    replaceAdminsInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => v.id === id
...
export const {
  ...
  replaceAdminsInList,
  clearAdminsList,
} = adminsListData.actions;

export default adminsListData.reducer;
