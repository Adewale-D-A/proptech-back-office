...
export const safetyAndSecurityData = createSlice({
  name: "all safety and security",
  initialState: {
    value: {
      status: false,
      pagination: [] as {
      ...
      data: [] as safetyAndSecurity[],
    },
  },
  reducers: {
    updateSafetyAndSecurity: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addSafetyAndSecurity: (state, action) => {
    ...
    removeSafetyAndSecurity: (state, action) => {
    ...
    replaceSafetyAndSecurity: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => v.id === id
...
export const {
  ...
  removeSafetyAndSecurity,
  replaceSafetyAndSecurity,
  clearSafetyAndSecurity,
} = safetyAndSecurityData.actions;

export default safetyAndSecurityData.reducer;
