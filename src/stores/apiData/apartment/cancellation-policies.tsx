...
export const cancellationPolicies = createSlice({
  ...
  initialState: {
    value: {
      ...
      pagination: [] as {
        ...
        data: canecllationPolicy[];
      }[],
      data: [] as canecllationPolicy[],
    },
  },
  reducers: {
    updateCancellationPolicyList: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addCancellationPolicyToList: (state, action) => {
    ...
    removeCancellationPolicyInList: (state, action) => {
    ...
    replaceCancellationPolicyInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => v.id === id
    ...
    clearCancellationPolicyList: (state) => {
...
export const {
  ...
  removeCancellationPolicyInList,
  replaceCancellationPolicyInList,
  clearCancellationPolicyList,
} = cancellationPolicies.actions;

export default cancellationPolicies.reducer;
