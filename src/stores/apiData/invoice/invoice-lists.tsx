...
export const invoiceListsData = createSlice({
  name: "all invoice",
  initialState: {
    value: {
      status: false,
  ...
  reducers: {
    ...
    removeInvoiceInList: (state, action) => {
    ...
    replaceInvoiceInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => String(v.id) === String(id)
...
export const {
  ...
  clearInvoiceList,
} = invoiceListsData.actions;

export default invoiceListsData.reducer;
