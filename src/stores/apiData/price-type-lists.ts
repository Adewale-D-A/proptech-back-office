...
export const priceTypeListsData = createSlice({
  name: "all price types",
  initialState: {
    value: {
      status: false,
      pagination: [] as {
  ...
  reducers: {
    updatePriceTypeList: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addPriceTypeToList: (state, action) => {
    ...
    removePriceTypeInList: (state, action) => {
    ...
    replacePriceTypeInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: string }) => v.id === id
...
export const {
  ...
  removePriceTypeInList,
  replacePriceTypeInList,
  clearPriceTypeList,
} = priceTypeListsData.actions;

export default priceTypeListsData.reducer;
