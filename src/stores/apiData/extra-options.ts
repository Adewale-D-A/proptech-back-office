...
export const extraOptionData = createSlice({
  name: "all extra options",
  initialState: {
    value: {
      status: false,
      pagination: [] as {
  ...
  reducers: {
    updateExtraOptions: (state, action) => {
      state.value.status = true;
      state.value.data = action?.payload?.data;
    },
    addExtraOption: (state, action) => {
    ...
    removeExtraOption: (state, action) => {
    ...
    replaceExtraOption: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => v.id === id
...
export const {
  ...
  replaceExtraOption,
  clearExtraOptions,
} = extraOptionData.actions;

export default extraOptionData.reducer;
