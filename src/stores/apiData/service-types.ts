...
export const serviceTypeData = createSlice({
  ...
  reducers: {
    ...
    addServiceTypeToList: (state, action) => {
    ...
    removeServiceTypeInList: (state, action) => {
    ...
    replaceServiceTypeInList: (state, action) => {
      const { id } = action?.payload;
      const currentArray = state.value.data;
      const currentIndex = currentArray.findIndex(
        (v: { id: number }) => String(v.id) === String(id)
...
export const {
  ...
  clearServiceTypes,
} = serviceTypeData.actions;

export default serviceTypeData.reducer;
